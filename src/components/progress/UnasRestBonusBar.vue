<template>
  <RestBonusBar
    :rest-bonus="char.unas_dailies.rest_bonus"
...
export default {
  ...
  methods: {
    changeUnasRestBonus(value) {
      const current =
        this.char.unas_dailies.rest_bonus.value === undefined ? 0 : this.char.unas_dailies.rest_bonus.value
      if (current + value > 100) {
        this.char.unas_dailies.rest_bonus.value = 100
      } else if (current + value < 0) {
        this.char.unas_dailies.rest_bonus.value = 0
      } else {
    ...
    updateUnasRestBonus(date) {
      this.char.unas_dailies.rest_bonus.last_update = date
      this.updateAndRefresh(this.characters)
    },
